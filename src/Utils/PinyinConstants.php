...
class PinyinConstants
{
     ...
     */
    const BASIC_CHINESE_RANGE = '\x{4E00}-\x{9FFF}';
    
    /**
     * 扩展A区汉字 (CJK Extension A)
     */
    const EXT_A_CHINESE_RANGE = '\x{3400}-\x{4DBF}';
    
    /**
     * 扩展B区汉字 (CJK Extension B)
     */
    const EXT_B_CHINESE_RANGE = '\x{20000}-\x{2A6DF}';
    
    /**
     * 扩展C区汉字 (CJK Extension C)
     */
    const EXT_C_CHINESE_RANGE = '\x{2A700}-\x{2B73F}';
    
    /**
     * 扩展D区汉字 (CJK Extension D)
     */
    const EXT_D_CHINESE_RANGE = '\x{2B740}-\x{2B81F}';
    
    /**
     * 扩展E区汉字 (CJK Extension E)
     */
    const EXT_E_CHINESE_RANGE = '\x{2B820}-\x{2CEAF}';
    
    /**
     * 兼容汉字范围 (CJK Compatibility Ideographs)
     */
    const COMPATIBLE_CHINESE_RANGE = '\x{F900}-\x{FAFF}\x{2F800}-\x{2FA1F}';
    
    /**
     * 完整的汉字 Unicode 范围（包含所有扩展区）
     */
    const FULL_CHINESE_RANGE = '\x{4E00}-\x{9FFF}\x{3400}-\x{4DBF}\x{20000}-\x{2A6DF}\x{2A700}-\x{2B73F}\x{2B740}-\x{2B81F}\x{2B820}-\x{2CEAF}\x{F900}-\x{FAFF}\x{2F800}-\x{2FA1F}';
    
    /**
     * 基本汉字范围（GB2312标准，约20902个常用汉字）
     */
    const GB2312_CHINESE_RANGE = '\x{4E00}-\x{9FA5}';
    
     /**
     * 匹配字母数字的范围
     */
    const ALPHANUMERIC_RANGE = 'a-zA-Z0-9';
     ...
     */
    const PATTERN_PURE_CHINESE_TEMPLATE = '/[%s]/u';
    
    /**
     * 匹配非汉字的正则模式模板（需要配合 sprintf 使用）
     */
    const PATTERN_NON_CHINESE_TEMPLATE = '/[^%s]/u';
     ...
     * 匹配拼音声调的正则模式
     */
    const PATTERN_PINYIN_TONE = '/[āáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ]/u';
    
    /**
     * 匹配无声调拼音的正则模式
     */
    const PATTERN_PINYIN_NO_TONE = '/^[a-z]+[1-5]?$/i';
     ...
     * 默认允许的特殊字符（在删除模式下保留）
     */
    const DEFAULT_SPECIAL_CHARS_ALLOWED = 'a-zA-Z0-9_\-+.';
    
    /**
     * 默认特殊字符映射表
     */
    const DEFAULT_SPECIAL_CHAR_MAP = [
        '，' => ',', '。' => '.', '！' => '!', '？' => '?',
        '；' => ';', '：' => ':', '（' => '(', '）' => ')',
        '【' => '[', '】' => ']', '《' => '<', '》' => '>',
        '「' => '"', '」' => '"', '『' => "'", '』' => "'",
